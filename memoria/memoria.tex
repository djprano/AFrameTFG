...
A continuación en la figura ~\ref{figura:mapaTecnologias}, muestro un mapa donde se puede visualizar como se relacionan las tecnologías y los servicios de datos principales en las que se basará el prototipo que voy a realizar. 
 \begin{figure}[h]
    \centering
    \includegraphics[width=16cm, keepaspectratio]{img/mapaFuncional.drawio.png}
...
\begin{table}[h]
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|c|c|c|c|c|c|}
			\hline
			\textbf{Índice} & \textbf{Propiedad} & \textbf{Tipo} & \textbf{Descripción} \\
			\hline
...
En resumen, \textsc{A-Frame} utiliza tecnologías como \textsc{HTML5} y componentes propios para definir la jerarquía del escenario y la \glsdisp{renderización}{renderización} de toda la escena.
\section{DOM}
\label{sec:dom}
\subsection{Introducción a la tecnología}
...
\end{center}
\subsection{Contexto y aplicación en el prototipo}
En este proyecto \textsc{JavaScript} ha sido el lenguaje de programación base utilizado para la implementación de toda la lógica de negocio desarrollada para el prototipo. Es por lo tanto la herramienta principal para el diseño y desarrollo de componentes que compondrán la escena final. A parte de esto como podréis ver en las secciones posteriores es la tecnología sobre la que está construida los \emph{\glsdisp{framework}{frameworks}} y librerías usados en este proyecto tales como \textsc{A-Frame}~\ref{sec:aframe}, \textsc{Leaflet}~\ref{sec:leaflet} y \textsc{Three.js}\ref{sec:threejs}.
\subsection{Ejemplos de uso}

\section{WebGL}
\label{sec:webgl}
\subsection{Introducción a la tecnología}
La tecnología de gráficos \textsc{\gls{3D}} es una tecnología que permite la \glsdisp{renderización}{renderización} de gráficos interactivos en tiempo real sobre un navegador web sin la necesidad de plugins adicionales. Está construida sobre una \textsc{\gls{API}} basada en \textsc{\gls{OpenGL}} para acceder a la funcionalidad de renderizado de la tarjeta gráfica del dispositivo.
...
\section{WebXR}
La tecnología \textsc{WebXR} es una agrupación de estándares que se utilizan en conjunto para habilitar la representación de escenas en 3D en \glsdisp{hardware}{hardware} diseñado para la visualización de mundos virtuales (VR) o para agregar gráficos al mundo real (realidad aumentada). \textsc{WebXR} proporciona una combinación perfecta entre realidad aumentada y realidad virtual.
...
La realidad virtual (\textsc{VR}) es la tecnología responsable de recrear un entorno ficticio que simula la apariencia real, permitiendo al usuario sumergirse en él y sentir que forma parte de ese entorno. Para experimentar esta realidad virtual, se requieren dispositivos especiales como las gafas o cascos de realidad virtual. Estos dispositivos generan entornos virtuales utilizando la combinación de \glsdisp{hardware}{hardware} y software diseñados para engañar a nuestros sentidos. Las imágenes en el visor se muestran de manera estereoscópica, lo que significa que se crean dos imágenes ligeramente diferentes para simular una perspectiva diferente en cada ojo. Esto crea una sensación de percepción tridimensional, similar a cómo percibimos el mundo real con nuestros sentidos.
\subsection{Contexto y aplicación en el prototipo}
...
\begin{itemize}
	\item \textbf{\emph{Renderizado en tiempo real}}: aprovechando la aceleración de \emph{\glsdisp{hardware}{hardware}} proporcionada por \textsc{WebGL}, \textsc{Three.js} es capaz de renderizar gráficos en \textsc{3D} de manera eficiente en los navegadores web modernos.
...
\label{sec:aframe}
\subsection{Introducción a la tecnología}
\textsc{A-Frame}\cite{aframedocs} es un \emph{\glsdisp{framework}{framework}} de código abierto construido sobre la tecnología \textsc{Three.js} de la que se habla en la sección~\ref{sec:threejs}, que permite la creación de experiencias de realidad virtual en entornos web. A diferencia de otros \emph{\glsdisp{framework}{frameworks}} \textsc{3D}, \textsc{A-Frame} ofrece un sistema de implementación sencillo, utilizando una sintaxis declarativa similar a la de \textsc{HTML5}. 
...
Además, se inició la captura de un \emph{\glsdisp{raster}{raster}} para incorporar una referencia en forma de mapa de \textsc{OpenStreetMap} en el suelo, lo cual permitirá verificar si se están creando las geometrías en el lugar correcto.
...
\subsubsection{Objetivos}
\begin{itemize}
    \item Generar un archivo \textsc{DEM} y un \emph{\glsdisp{raster}{raster}} para los escenarios de Madrid y Vatry.
...
\end{itemize}
\subsubsection{Desarrollo}
Se generan los ficheros \textsc{DEM} y \emph{\glsdisp{raster}{raster}} con éxito siguiendo el procedimiento descrito en las secciones \ref{sec:dem} y \ref{sec:raster}. 
...
\subsubsection{Resultado}
Se puede observar en la figura \ref{fig:demAframe} cómo cargo una demostración simple con los ficheros generados del binario de alturas con su respectivo \glsdisp{raster}{raster}. 
...
En este punto, me enfrento al primer problema que debo abordar en las conversiones. Si observamos una vista aérea tanto del suelo de nuestro escenario como de un mapa representado por un \glsdisp{datum}{datum} \textsc{WGS84} y proyectado con \textsc{Mercator}, notaremos una diferencia en las coordenadas.
La coordenada \textsc{Y}, que representa la latitud en el sistema \glsdisp{geoespacial}{geoespacial}, aumenta hacia arriba, mientras que en el sistema tridimensional, se representa a lo largo del eje \textsc{Z} y aumenta en dirección opuesta. Por lo tanto, esto es algo que debo tener en cuenta al diseñar las fórmulas de conversión.
Si observamos la figura \ref{fig:sistemasDeReferencia}, se puede apreciar cómo la posición con coordenada \texttt{(x=8, y=-7)} en el sistema \glsdisp{geoespacial}{geoespacial} se representa de manera diferente en ambos sistemas de referencia.
\begin{figure}[h]
  \centering
...
También se podrá encontrar las entidades que serán colocadas sobre el suelo, como la cámara principal que se moverá sobre la superficie del terreno y los edificios que deben estar alineados con el suelo. Para modelar las superficies, se utilizan conjuntos de datos \glsdisp{raster}{raster}. Un raster es una matriz compuesta por celdas o píxeles dispuestos en filas y columnas que cubren una determinada región geográfica.
...
Un archivo DEM es un tipo de archivo raster con celdas que representan un valor de elevación correspondiente a una ubicación geodésica específica en relación a un \glsdisp{datum}{datum} de referencia. En resumen, un archivo DEM es una representación muestreada de una superficie terrestre limitada, donde las alturas se almacenan en una matriz.
...
Para extraer la información \glsdisp{geoespacial}{geoespacial} de los edificios, me he basado en el servicio proporcionado por \textsc{Overpass}\footnote{\url{https://overpass-api.de/}}, el cual ofrece acceso a datos geoespaciales almacenados en la base de datos de \textsc{OpenStreetMaps}\footnote{\url{https://www.openstreetmap.org/}}. 
...
Para calcular la altura del terreno, se obtiene el \glsdisp{centroide}{centroide} de la lista de puntos que definen el contorno del edificio. Luego, se aplican las fórmulas descritas en \ref{formula:calculoIndiceAlturas} para determinar la altura del terreno en el punto del centroide.
...
Después de explorar diversas estrategias, se llega a la conclusión de que la técnica más sencilla y eficiente es utilizar un \emph{\glsdisp{raycaster}{raycaster}}. Un componente \emph{raycaster} es una herramienta fundamental en entornos \textsc{3D} que permite realizar intersecciones y detecciones de colisiones en la escena. Su funcionamiento se basa en el trazado de un rayo desde un origen en una dirección determinada. Cuando el rayo colisiona con las entidades envía eventos a los que los componentes pueden suscribirse.\\
\begin{figure}[h]
	\centering
...
\newglossaryentry{datum}{
	name={DATUM},
	description={En el campo de la geodesia, un datum se refiere a un conjunto de puntos de referencia en la superficie terrestre que se utilizan para realizar mediciones de posición. Además, está asociado a un modelo que describe la forma de la Tierra, conocido como elipsoide de referencia, y se utiliza para establecer un sistema de coordenadas geográficas. En resumen, un datum proporciona los puntos de referencia y el marco de referencia necesario para definir y utilizar un sistema de coordenadas geográficas en mediciones de posición en la Tierra.}
}
\newglossaryentry{w3c}{
	name={W3C},
	description={El World Wide Web Consortium (W3C) es una organización internacional que desarrolla estándares y protocolos para la web, con el objetivo de garantizar la interoperabilidad y promover el crecimiento de la web.}
}
